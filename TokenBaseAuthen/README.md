...
## Method Length:
If a method becomes very long it is hard to understand. Therefore long methods should usually be refactored into several individual methods that focus on a specific task. While there is no fixed rule on how long a method can be but it is good to keep method length within 100-150 lines (would be better to keep the length < 20 lines!).
